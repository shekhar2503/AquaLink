...
**Canonical file:** `AquaLink/backend/data/aqualink_dataset.csv`
**Published frontend fallback:** `aqualink-dashboard/public/aqualinkData.csv`
**Rows:** 44,430 | **Columns:** 31
**Coverage:** 36 districts → 368 synthetic taluka identifiers → 7,405 synthetic village/ward identifiers → 6 years (2020–2025)
...
| `Recommended_Action` | Rule-based recommendation produced by `backend/app/core/engine.py` |
...
## Recalculation and publication process

The canonical CSV is the only dataset edited directly. The engine-owned columns are:
...
Rebuild them and publish the frontend fallback from the repository root:

```powershell
python AquaLink\backend\scripts\rebuild_dataset.py
python AquaLink\backend\scripts\validate_dataset.py
```

The rebuild script calls the current scoring engine for every row, creates a timestamped local backup under `backend/data/backups`, rewrites the canonical CSV atomically, and then writes the complete canonical dataset to the frontend fallback. The fallback is therefore an exact publication of all canonical rows and years rather than a separately maintained extract.

The validation command fails when it detects schema changes, missing values, invalid ranges or categories, duplicate location/year keys, unstable location identity fields, engine-owned field differences, or any difference between the canonical and fallback CSVs. It writes a concise machine-readable report to `backend/data/validation_summary.json`.

### 2025 repair

Repository history contains the original complete CSV but no source-generation script for the 2025 append. A consistency audit found that its engine-owned fields had been produced by logic that did not match the current engine. On 2026-09-24, all engine-owned columns across 2020–2025 were recalculated with the current engine. This materially repaired 2025 and also normalized earlier one-decimal rounding and recommendations generated by the older ruleset. Raw indicators and location identity fields were not changed by this repair.
...
- Synthetic taluka identifiers must be displayed as `Synthetic area group NN`; they are not mappings to real Pune talukas, and no LGD codes or administrative relationships are assigned.
...
- Coordinates are simulated and must not be interpreted as verified monitoring sites.
- A coordinate audit against the repository's undocumented mixed-vintage (`2011_c` / `update2014`) district boundary file finds 412 of 7,405 unique locations outside its Maharashtra polygons. Five of 250 Pune locations fall outside its Pune polygon. This reference comparison does not verify any synthetic point.
- The repository contains no sourced, usable Pune taluka boundary layer, so taluka choropleth mode is disabled.
- The repository does not contain the original raw-data generator for the 2025 indicators. The current rebuild process is reproducible only for engine-owned derived fields.
- Real deployment should replace this with CGWB WRIS/INDIA-WRIS API + JJM IMIS dashboard exports + GSDA data.

## Synthetic geography display and aggregation

The API derives stable AquaLink-only IDs (`syn-location-*` and `syn-area-*`), safe display labels, `synthetic_unverified` geography status, and coordinate-source/status fields. These are not government identifiers.

Latest-year synthetic area-group averages are arithmetic means of location-level `Water_Stress_Score`. A group “containing a hotspot” is classified by its highest individual location risk. The UI keeps those two concepts separate; neither is a verified taluka statistic.
